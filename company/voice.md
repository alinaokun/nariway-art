...
**Substack Notes.** The same voice, made simple and easy to read. Keep the curatorial judgment and the provenance detail, but write plainly, plain words, one idea per sentence, sentences a person would say out loud. Two things to avoid at the edges. No fragment stacks (three clipped stubs in a row), which read as machine-written and violate the staccato ban. And no dense, clause-heavy periodic sentences either, which read as stiff. The target sits between them, clear complete sentences of moderate length. A Note is two or three beats with a blank line between them, so the feed gets its white space from the beat breaks rather than from chopping sentences. A specific fact in beat one, plain and direct rather than casual. One idea. **End on a concrete fact or image, never a manufactured conclusion.** The failure mode to avoid is the summarizing moral, especially the antithesis kicker ("it was never about X, it was about Y", "the building was never the achievement, it was the start of the problem"), which reads as forced wisdom and breaks the voice guide's own rule against stating the meaning. If a closing line explains what the story means, cut it and let the last fact land. Never a closing question. The em-dash ban is free here since the beat break does that work. Written this way a Note runs a little above the platform's 60-word optimum, an accepted cost of sounding like Alina. Format details in [[substack]].
