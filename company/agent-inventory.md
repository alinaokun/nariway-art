...
- **cfo-steward — TURN OFF (Alina's decision 2026-08-27).** Still running (`trig_014KCjwMrTLo85e1sKudTUof`). A non-owning agent session cannot disable it via the trigger API (confirmed 2026-08-28: "Agents can only update routines they created... A routine's own session may still disable itself"), and its own daily run has not yet self-disabled. Needs either a manual flip at claude.ai/code/routines, or a one-time edit to its stored prompt telling it to self-disable on next fire.
...
- **2026-08-28** — cmo-steward self-disabled on its own scheduled run (see above); this was its last run, no more old-model marketing entries land on autopilot. cfo-steward remains on, needs its own self-disable or a manual flip; a non-owning agent cannot do it for it.
