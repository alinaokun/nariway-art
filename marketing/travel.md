...
*A work-function (no persona; the first bit of operations — as load grows it may become a named Operations role). Consumes the [[events-radar]] (which decides *what* is worth attending) and handles *getting there*: drive vs. fly, routing, lodging, calendar, deadlines, cost. **Home base: Montvale, NJ.** The Travel function proposes; Alina books.*

## Travel plan (from events-radar; confirm dates before booking)
| Event | Date | From Montvale | Lodging | Est. all-in | Decision |
|---|---|---|---|---|---|
...
## Personal trips (coordinate Nariway-relevant visits)
Alina's own travel, where the Travel function looks for on-thesis museums, collections, and people to fold in. Anchors below are the strongest collector-founded and house-museum ties from general knowledge, **confirm hours, current exhibitions, and booking before the trip** (several require advance passes). Offer per-trip detailed plans on request.

- **Atlanta, GA — Sept 2 to 8, 2026.** Advance to-dos:
  1. **Reach out to [[Souls Grown Deep Foundation]] now** (by mid-August). It is a foundation office, not a walk-in museum, so a visit must be arranged, and this is Alina's own case, a possible discovery relationship, not only a viewing. Draft email is in the note.
  2. **High Museum of Art** — check the September exhibitions (strong American, folk, and self-taught holdings; has shown Souls Grown Deep works). Just buy tickets, no advance needed.
  3. Optional, if time: the **Michael C. Carlos Museum** (Emory) and the **Hammonds House Museum** (African American art). Confirm before relying on any specifics.
...
A verified register of smaller collections, house museums, and collector-founded institutions within a drive of Montvale lives in [[regional-visits]], the on-thesis places to see without a plane. Closest starts worth the drive: Caramoor (Katonah, ~50 min), Kykuit (Tarrytown, ~40 min), Montclair (~30 min), and Magazzino (Cold Spring, ~1.25 hrs).
...
- NYC Nov week: plan now (some conferences take proposals in the fall).
