...
1. **Post the first Substack Note** — the queue is playbook-clean (the CMO steward fixed all four closings this week), and it has now sat ready and unposted for four straight days *while Query A is live and drawing eyes to the publication.* Both CFO and CMO steward name this as the single highest-leverage move outstanding. → [[substack-notes-queue]]
2. **Confirm whether nariway.com's interim art-collections copy is actually live** — this week's first site-health run found Google's index still shows the old "Hidden History of Work" identity site-wide. If the swap never went live, that's the fix; if it did, this is a reindex-only request (Search Console, once connected) — either way, a Qwoted respondent googling Nariway right now may land on the wrong site. → [[site-health]]
3. **Query A's response window is open** — reply with the templates and log every substantive response in the Intelligence Log as they land (the log is still empty). → [[conversations-series]]
4. **One fact only you know:** does Alina's own bio say "more than 100 museums" or "hundreds of museums"? `website.md` and `positioning.md` still disagree. A one-line reply picks the true figure so the drift can be fixed.
5. **A QA judgment call, not yet acted on:** the live, published Substack About/Welcome copy in `positioning.md` runs several short parallel sentences and subject-less fragments in a row, against `voice.md`'s own staccato/fragment rules. Worth a look next time either page is touched, since it's live public copy, not a draft. → `company/qa-report.md` #2
6. **Send the Vogel outreach** — Ruth Fine first, then the four museum inquiries. Both drafts are ready and waiting in §8 of the afterlife file; this only goes out when you send it. → [[vogel-fifty-states-afterlife]]
7. **Compile the conversations pipeline** (your call, in progress) — pick the first three across different perspectives (Phillip Collins = Builder, Ruth Fine = Institution, one Advisor from Query A or the warm network). Nothing gets built until you choose them. → [[conversations-series]] *([[work-log]] is a location-only tax record, default Montvale, no action needed unless you work out of state.)*
...
- **CFO (the money).** The day the response window actually opened, founder time went straight back to research and writing (a flagship-report layout rebuild, a new market-intelligence entry, a publishing-architecture revision) rather than commercial validation — the CFO steward's honest drift flag. Still $0 earned, still no client conversation since Dawn on Aug 13.
- **CMO (audience).** The Note-closings fix is done (all four now end on a concrete fact, per this week's QA catch) — the queue is playbook-clean. What hasn't moved: still unposted, day four running, and the website watch is now a fourth day unresolved, escalated this week from "probably stale cache" to a first confirmed site-health CRITICAL.
- **Research (the case).** Held at 50 of 50, depth-only. Two ecosystem chapters got their first real sourcing this run: the art-finance mechanics (LTV terms, and the auction houses' own lending desks — see Today I Learned) and the institutional-economics side of "why museums say no" (a 90–95% gift-rejection rate, a live Philadelphia Museum deficit, AAM's 2025 sector survey). WebFetch stayed blocked for a ninth-plus straight run.
- **QA (the record).** Two criticals this week: the governance-survival stat had lost its "not a population rate" caveat as it traveled into this page and the daily email — now patched at the source in both places. The second, live Substack copy breaking `voice.md`'s own sentence-shape rules, is still open (above).
- **The one-million gap.** Unchanged: a solo advisory caps around 300–700k, so the target still needs a scalable leg. Query A's response window is open and the Intelligence Log is still empty — that silence is itself the current data point on H7B, not yet a signal either way.

---

## Where things stand

- Cases mapped, about 80. Coded to report grade, **50 of 50** (target reached 2026-08-16), and future runs shift entirely to depth — primary verification once WebFetch unblocks, and cross-case analysis like this run's first fully-counted governance-survival table. WebFetch is still blocked on primary sources (tenth consecutive confirmation) — most cases remain secondary-sourced pending a re-fetch pass.
...
- **New direction, the flagship report.** An annual, data-rich "State of art and legacy" report (à la the State of AI Report), built on the case database, to make Nariway the source at art and finance. This is the scalable leg toward a million. Phased so it never becomes a black hole, and it runs alongside the market conversations, not instead. **Watch it evolve daily → [[what-becomes-of-great-art-collections]]** (the living manuscript, now in a clean WEF-style layout; the [[flagship-report]] page holds the plan). Two of the surrounding-ecosystem chapters got their first real sourcing this week (art finance §4.3, institutional economics §4.5); macro, insurance, and tax are still scaffolded but unfilled.
